...
class MyTestCase(unittest.TestCase):
    ...
    def test_qywechat_robot_detail(self):
        #       valuation table detail
        # v_json = pd.read_json(valuations,orient='records')
        # print(v_json)
        pd.DataFrame(json.loads(valuations))
        v1 = json.dumps(valuations)
        train = pd.DataFrame.from_dict(v1, orient='index')
        robot_detail = {
            "msgtype": "image",
            "image": {
                "base64": "",
        ...
        robot_detail_result = requests.post(robot, data=json.dumps(robot_detail),
                                            headers={"Content-Type": "application/json"}).json()
        self.assertEqual(robot_detail_result['errcode'], 0)
